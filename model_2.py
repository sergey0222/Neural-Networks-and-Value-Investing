...
with open(path + 'filter_2/filter_2_sub.txt') as f:
...
with open(path + 'filter_2/filter_2_num.txt') as f:
        f_object = csv.reader(f, delimiter='\t')
        for row in f_object:
           tag = row[1]
...
# create mask to extract reports for which  Full Year report exist for year_to_predict
mask1D = XY_bool[:, (year_to_predict - first):(year_to_predict - first + 1)]
